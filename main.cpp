...
public:
	...
	void addedge(int v1,int v2){
      if(!Adj[v1].IsinList(v2)) Adj[v1].Append(v2);
      if(!Adj[v2].IsinList(v1)) Adj[v2].Append(v1);
	}
...
int main() {
	...
	std::getline(std::cin, line);
	    ...
	    case 'E':
	    	...
	    	argE=new List;
	    	tmp=arg->get_head();
	    	if(!tmp){empty_edge_flag=1;state='V'; break;}
	    	do{
	    ...
	    case 's':
	    	...
	    	if(!argE->IsinList(first) || !argE->IsinList(second) ) {
	    		std::cerr << "Error: " << "there is no path between vertices" <<std::endl;
	    		delete arg;
	    		break;
	    	}
