...
public:
	...
	Line(Point s, Point d) :
			src(s), dst(d) {
		length = pow((dst.y - src.y), 2) + pow((dst.x - src.x), 2);
	}
	friend class Street;
...
public:
	...
	void add() {
		...
		for (int i = 0; i < size; i++) {
			if (i == 0) {
				std::cout << "(" << std::to_string(segment[i].src.x) << ","
						<< std::to_string(segment[i].src.y) << ") ";
			}
			std::cout << "(" << std::to_string(segment[i].dst.x) << ","
					<< std::to_string(segment[i].dst.y) << ") ";
		}
		std::cout << std::endl;
	}
